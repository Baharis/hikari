...
def completeness_map(a, b, c, al, be, ga,
    ...
    log = open(lst_path, 'w+', buffering=1)
    lst = open(lst_path, 'w+', buffering=1)

    def make_reference_ball():
        q = HklFrame()
        q.crystal.edit_cell(a=a, b=b, c=c, al=al, be=be, ga=ga)
        q.edit_wavelength(wavelength)
        q.make_ball(radius=min(p.r_lim, 1/resolution))
        q.extinct('000')
        for extinction in extinctions:
            q.extinct(*extinction.split(':', 1), laue_group=laue_group)
        return q
    p = make_reference_ball()
    total_reflections = len(p)

    def determine_theta_and_phi_limits():
